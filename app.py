...
BACKEND_URL = "https://sam-suppliers-backend.onrender.com"
...
if not st.session_state.logged_in:
    ...
    st.markdown("Please enter your registered telephone number and password to access the system.")
    ...
    with st.form("login_form"):
        ...
        if submit_login:
            if phone_input and password_input:
                try:
                    res = requests.post(f"{BACKEND_URL}/worker/login", json={
                        "phone": phone_input,
        ...
        with st.form("new_worker_form"):
            ...
            if st.form_submit_button("Add Worker"):
                if w_name and w_phone and w_pass:
                    w_res = requests.post(f"{BACKEND_URL}/workers", json={
                        "name": w_name, "phone": w_phone, "password": w_pass, "role": w_role
                    })
                    if w_res.status_code == 200:
                        st.success("Worker registered successfully!")
                    else:
                        st.error(w_res.json().get("detail", "Error adding worker"))
                else:
...
tabs = st.tabs(["Point of Sale", "Business Dashboard", "Inventory / Products", "Customer Management", "Credit Management", "Expenses"])
        ...
        if prod_res.status_code == 200 and cust_res.status_code == 200:
            ...
            if not products:
                ...
                with st.form("add_to_cart_form"):
                    selected_prod_name = st.selectbox("Select Product", list(prod_dict.keys()), key="pos_prod")
                    selected_prod = prod_dict[selected_prod_name]
                    
                    if selected_prod.get("is_low_stock"):
                        st.warning(f"⚠️ Low Stock Warning: Only {selected_prod.get('stock_display')} remaining!")
                    else:
                        st.info(f"Available Stock: {selected_prod.get('stock_display', 'N/A')}")
                    
                    unit_type = st.selectbox("Unit Type", ["piece", "box"], key="pos_unit")
                    price_type = st.selectbox("Price Type", ["retail", "wholesale"], key="pos_price")
                    ...
                    if add_to_cart_btn:
                        ...
                        st.success(f"Added {quantity} {unit_type}(s) of {selected_prod['name']} to order!")
                ...
                if st.session_state.cart:
                    ...
                    for idx, item in enumerate(st.session_state.cart):
                        ...
                        if p_item:
                            if item["unit_type"] == "box":
                                u_price = p_item["box_pricing"]["wholesale_price_per_box"] if item["price_type"] == "wholesale" else p_item["box_pricing"]["retail_price_per_box"]
                            else:
                                u_price = p_item["wholesale_price_per_base"] if item["price_type"] == "wholesale" else p_item["retail_price_per_base"]
                            line_total = u_price * item["quantity"]
                            subtotal += line_total
                            
                            c1, c2, c3, c4 = st.columns([3, 2, 2, 1])
                            c1.write(f"**{item['product_name']}** ({item['price_type']})")
                    ...
                    discount = st.number_input("Discount Amount (UGX)", min_value=0.0, value=0.0, step=500.0)
                    grand_total = max(0.0, subtotal - discount)
                    st.markdown(f"**Final Total after Discount:** UGX {grand_total:,.0f}")
                    
                    payment_status = st.selectbox("Payment Method", ["Cash", "Credit", "Partial"], key="pos_pay_status")
                    if payment_status == "Cash":
                        amount_paid = float(grand_total)
                    elif payment_status == "Credit":
                        amount_paid = 0.0
                    else:
                        amount_paid = st.number_input("Amount Paid Now (UGX)", min_value=0.0, max_value=float(grand_total), value=0.0, step=500.0)
                    
                    balance_due = grand_total - amount_paid
                    if balance_due > 0:
                        st.warning(f"Balance Left as Credit: UGX {balance_due:,.0f}")
                        
                    delivery_option = st.selectbox("Delivery Option", ["Pick up", "Customer delivery personnel", "Own delivery"], key="pos_delivery")
                    
                    cust_dict = {c["name"]: c["id"] for c in customers}
                    cust_names = ["Walk-in Customer"] + list(cust_dict.keys())
                    selected_cust_name = st.selectbox("Customer for Order", cust_names, key="pos_cust")
                    customer_id = None if selected_cust_name == "Walk-in Customer" else cust_dict[selected_cust_name]
...
"""
                            ...
                            st.error(res.json().get("detail", "Error processing checkout"))
                else:
                    st.info("Your cart is currently empty. Add products above to build a multi-item order.")
        else:
...
with tabs[1]:
    ...
    try:
        ...
        if dash_res.status_code == 200:
            ...
            col2.metric("Total Credit Owed", f"UGX {data['credit_owed_by_customers']:,.0f}")
        ...
        if sales_res.status_code == 200:
            ...
            if sales_list:
                search_date = st.text_input("Filter by Date (YYYY-MM-DD or leave blank)", "")
                filtered_sales = [
                ...
                for s in filtered_sales:
                    with st.container():
                        ...
                        c1.write(f"**{s.get('product_name')}**\n\n`{s.get('sale_date')}`")
                        c2.write(f"Qty: {s.get('quantity_sold')} {s.get('unit_sold')}(s)")
                        c3.write(f"Total: UGX {s.get('total_price'):,.0f}\nDisc: UGX {s.get('discount', 0):,.0f}")
                        paid = s.get('amount_paid', 0)
                        bal = (s.get('total_price') - s.get('discount', 0)) - paid
                        c4.write(f"Paid: UGX {paid:,.0f} ({s.get('payment_status')})\nBal: UGX {bal:,.0f}")
                        c5.write(f"Cust: **{s.get('customer_name')}**\nAddr: *{s.get('customer_address')}*\nDel: {s.get('delivery_option')}")
                        
                        # Admin-only delete check
                        if st.session_state.role == "Admin":
                            if c6.button("Delete", key=f"del_sale_{s['id']}"):
                                del_res = requests.delete(f"{BACKEND_URL}/sales/{s['id']}?role=Admin")
...
with tabs[2]:
    ...
    with st.expander("➕ Add New Product"):
        with st.form("add_product_form"):
            ...
            c1, c2 = st.columns(2)
            pieces_per_box = c1.number_input("Pieces per Box", min_value=1, value=24, step=1)
            boxes_in_stock = c2.number_input("Boxes in Stock", min_value=0, value=10, step=1)
            
            st.markdown("**Box Prices**")
            c3, c4 = st.columns(2)
            wholesale_price_per_box = c3.number_input("Wholesale Price per Box", min_value=0.0, value=3600.0, step=100.0)
            retail_price_per_box = c4.number_input("Retail Price per Box", min_value=0.0, value=4800.0, step=100.0)
            
            st.markdown("**Custom Piece Prices**")
            c5, c6 = st.columns(2)
            wholesale_price_per_base = c5.number_input("Wholesale Price per Piece", min_value=0.0, value=150.0, step=10.0)
            retail_price_per_base = c6.number_input("Retail Price per Piece", min_value=0.0, value=200.0, step=10.0)
                ...
                with st.container():
                    ...
                    with col_a:
                        if p.get("is_low_stock"):
                            st.markdown(f"🔴 **{p['name']}** — Stock: **{p.get('stock_display')}** *(LOW STOCK)*")
                        else:
                            st.markdown(f"🟢 **{p['name']}** — Stock: {p.get('stock_display')}")
                        st.text(f"Box Price -> Retail: UGX {p['retail_price_per_box']:,.0f} | Wholesale: UGX {p['wholesale_price_per_box']:,.0f}")
                        st.text(f"Piece Price -> Retail: UGX {p['retail_price_per_base']:,.0f} | Wholesale: UGX {p['wholesale_price_per_base']:,.0f}")
                    
                    with col_b:
                        with st.form(f"restock_form_{p['id']}"):
                            st.write("Quick Restock")
                            r_boxes = st.number_input("Add Boxes", min_value=0, value=0, step=1, key=f"rb_{p['id']}")
                            r_pieces = st.number_input("Add Pieces", min_value=0, value=0, step=1, key=f"rp_{p['id']}")
                            if st.form_submit_button("Update Stock"):
                                restock_payload = {"boxes_to_add": r_boxes, "pieces_to_add": r_pieces}
                                res = requests.post(f"{BACKEND_URL}/products/{p['id']}/restock", json=restock_payload)
                                if res.status_code == 200:
...
with tabs[3]:
    ...
    with st.form("add_cust_form"):
        ...
        c_opening_credit = st.number_input("Pre-existing Opening Credit Owed (UGX)", min_value=0.0, value=0.0, step=1000.0)
        sub_cust = st.form_submit_button("Save Customer")
        if sub_cust:
            if c_name:
                res = requests.post(f"{BACKEND_URL}/customers", json={
                    "name": c_name, "phone": c_phone, "address": c_address, "opening_credit": c_opening_credit
                })
                if res.status_code == 200:
...
with tabs[4]:
    ...
    try:
        ...
        if custs:
            ...
            selected_cr_name = st.selectbox("Select Customer to Manage Credit", list(cust_dict_cr.keys()), key="cr_cust_sel")
            selected_customer = cust_dict_cr[selected_cr_name]
            
            st.metric("Total Outstanding Credit Owed", f"UGX {selected_customer.get('total_credit_owed', 0):,.0f}")
            st.write(f"**Phone:** {selected_customer.get('phone', 'N/A')} | **Address:** {selected_customer.get('address', 'Walk-in')}")
            
            with st.form("clear_credit_form"):
                amount_cleared = st.number_input("Amount Paid / Cleared Now (UGX)", min_value=0.0, value=0.0, step=500.0)
                sub_clear = st.form_submit_button("Record Credit Payment")
                if sub_clear:
                    if amount_cleared > 0:
                        res = requests.post(f"{BACKEND_URL}/customers/{selected_customer['id']}/clear-credit", json={"amount_cleared": amount_cleared})
                        if res.status_code == 200:
...
# --- TAB 6: EXPENSES ---
with tabs[5]:
    st.subheader("Shop Expenses Tracking")
    with st.form("add_expense_form"):
        worker_name = st.text_input("Worker Responsible")
        reason = st.text_input("Reason / Description for Expense")
        amount = st.number_input("Amount (UGX)", min_value=0.0, value=0.0, step=500.0)
        sub_exp = st.form_submit_button("Record Expense")
        if sub_exp:
            if worker_name and reason and amount > 0:
                res = requests.post(f"{BACKEND_URL}/expenses", json={
                    "worker_name": worker_name, "reason": reason, "amount": amount
                })
                if res.status_code == 200:
                    st.success("Expense recorded successfully!")
                    st.rerun()
                else:
                    st.error("Error recording expense.")
            else:
